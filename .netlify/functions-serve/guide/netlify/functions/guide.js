...
// netlify/functions/utils/guideLoader.js
...
var dataDir = import_node_path.default.join(__dirname, "..", "data");
var generatedGuidesDir = import_node_path.default.join(__dirname, "..", "..", "generated-guides");
function loadGuide(packageId) {
  try {
    const jsonGuidePath = import_node_path.default.join(dataDir, "examples", `${packageId}.json`);
    if (import_node_fs.default.existsSync(jsonGuidePath)) {
      const rawData = import_node_fs.default.readFileSync(jsonGuidePath, "utf8");
      return JSON.parse(rawData);
    }
    const txtGuidePath = import_node_path.default.join(generatedGuidesDir, `${packageId}.txt`);
    if (import_node_fs.default.existsSync(txtGuidePath)) {
      const rawData = import_node_fs.default.readFileSync(txtGuidePath, "utf8");
...
// netlify/functions/utils/pubdevApi.js
...
var DATA_DIR = import_node_path2.default.join(__dirname2, "..", "data");
var INTENTS_DIR = import_node_path2.default.join(DATA_DIR, "intents");
var PACKAGES_DIR = import_node_path2.default.join(DATA_DIR, "packages");
...
async function callGeminiForGuide(prompt) {
  try {
    initializeGemini();
    if (!model) {
...
// netlify/functions/utils/guideGenerator.js
