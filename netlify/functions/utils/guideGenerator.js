...
### 2. 단계 구성 (최소 6단계, 권장 8단계 이상)
- **Step 1**: 설치 + 기본 개념 + 첫 번째 동작하는 예제
- **Step 2-3**: 핵심 기능을 실제 앱에 적용하는 방법
- **Step 4-5**: 고급 패턴, 성능 최적화, 에러 핸들링
- **Step 6-7**: 실전 프로젝트 예제 (실무에서 자주 쓰는 패턴)
- **Step 8**: 테스트 작성 또는 디버깅 방법

### 3. 코드 품질 (매우 중요!)
- 모든 code.content는 **바로 복사해서 동작하는 완전한 코드**
- **한국어 주석으로 각 줄이 무엇을 하는지 상세히 설명**
- **최소 60줄 이상의 실질적인 코드** (짧은 코드는 절대 안됨!)
- import문, 클래스 전체, 필요한 모든 메서드 포함
- **각 함수/메서드마다 해당 함수가 무엇을 하는지, 왜 필요한지, 파라미터는 무엇인지 주석으로 상세 설명**

### 4. 함수별 상세 설명 (중요!)
각 코드 블록에서 다음을 반드시 포함:
\`\`\`
// ============================================
// 함수명: functionName
// 목적: 이 함수가 왜 필요한지 설명
...
### 5. 에러와 베스트 프랙티스
- **commonErrors**: 에러 메시지, 원인(cause), 해결책(solution) 모두 포함 (최소 4개)
- **bestPractices**: 실무에서 꼭 지켜야 할 패턴 5개 이상
- **tips**: 개발 생산성을 높이는 꿀팁 5개 이상

### 6. 출력 형식
- 반드시 순수 JSON만 출력 (마크다운 코드블록 없이)
- 위 예시와 동일한 JSON 구조 유지
- **분량: 위 예시의 150% 이상 상세하게 작성**

### 7. 실제 사용 시나리오 포함
각 단계에서 "실제로 이 기능은 언제 쓰는가?"를 명확히 설명하세요:
- 예: "이 패턴은 결제 페이지에서 사용자가 입력을 완료하기 전까지 버튼을 비활성화할 때 씁니다"
- 예: "이 기능은 채팅 앱에서 새 메시지가 올 때 자동 스크롤할 때 필수입니다"

### 8. 공식 예제 코드 분석 (중요!)
${hasExample ? `
위에 제공된 공식 예제 코드를 철저히 분석하고:
1. 예제에 나온 모든 클래스/함수를 가이드에 반영
2. 예제의 핵심 패턴을 단계별 가이드에 녹여내기
3. 예제에서 사용된 API들의 용도를 상세히 설명
` : '공식 예제가 없으므로 패키지의 README와 일반적인 사용 패턴을 기반으로 작성'}

이제 "${pkg.name}" 패키지에 대한 **초심자도 이해할 수 있으면서도 실무에 바로 적용 가능한 상세 구현 가이드**를 JSON으로 출력하세요:`;
}
